...
import { defRigor } from '../../../../../const'
import { updateAllBaselineInfluenceScores, updateBaselineInfluence } from '../../../../../redux/features/profiles/slice'
...
const InfluenceCalculations = () => {
  ...
  const defaultUserConfidence_ = Number(
    useSelector((state) => state.grapevine.controlPanels.baseLayer.defaultUserScore.confidence) / 100,
  )
  ...
  const muteInterpretationConfidence_ = Number(
    useSelector((state) => state.grapevine.controlPanels.baseLayer.muteInterpretation.confidence) / 100,
  )
  ...
  return (
    <div style={{ border: '1px solid orange', borderRadius: '5px', padding: '5px' }}>
      <center>Base Layer Influence Calculations</center>
      <div>dunbarNumber: {dunbarNumber}</div>
      <div>attenuationFactor_: {attenuationFactor_}</div>
      <div>aMyFollows.length: {aMyFollows.length}</div>
      <div>number of profiles: {Object.keys(oProfileData).length}</div>
      <div>aNearbyProfilesByPubkey length: {aNearbyProfilesByPubkey.length}</div>
      <div>oProfilesByPubkey length: {Object.keys(oProfilesByPubkey).length}</div>
      <div>
        convertInputToCertainty: {convertInputToCertainty(defaultUserConfidence_, rigor_)}
      </div>
      <div>
