...
const TanstackReactTable = ({ aNpubsToDisplay, oProfilesByNpub }) => {
  // const aNpubsToDisplay = Object.keys(oProfilesByNpub)
  const [data, _setData] = React.useState(() => [...createData({ oProfilesByNpub, aNpubsToDisplay })])
  ...
  return (
    <div className="p-2">
      <div>TanstackReactTable; oProfilesByNpub: {Object.keys(oProfilesByNpub).length}</div>
      <div>{totalRows} Rows</div>
      <div className="inline-block border border-black shadow rounded" style={{ float: 'right' }}>
